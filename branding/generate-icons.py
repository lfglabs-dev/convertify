...
def create_gradient_background(size):
    ...
    Vertical gradient from teal (top) to deep purple (bottom).
    """
    img = Image.new("RGBA", (size, size), (0, 0, 0, 255))

    # Colors from icon.json (Display P3 converted to sRGB)
    # Top: display-p3:0.14763,0.28478,0.38687 → RGB(38, 73, 99)
    # Bottom: display-p3:0.14291,0.13515,0.25242 → RGB(36, 35, 64)
    color_top = (38, 73, 99)
    color_bottom = (36, 35, 64)

    # Gradient stops at y=0.7 per icon.json, then solid bottom color
    gradient_end = 0.7
    ...
    for y in range(size):
        ...
        if ty <= gradient_end:
            # Interpolate within gradient region
            t = ty / gradient_end
...
def create_icon(shadow_path, logo_path, output_path, size, scale):
    ...
    logo_with_shadow = add_drop_shadow(
        logo_resized,
        logo_size,
        offset=(0, int(actual_size * 0.015)),  # Subtle vertical offset only
        blur_radius=int(actual_size * 0.02),  # Subtle blur
        shadow_opacity=0.05,  # Very subtle shadow
    )
...
def main():
    # Paths relative to branding folder
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    
    # Source assets in branding folder
    shadow_path = os.path.join(script_dir, "shadow.png")
    logo_path = os.path.join(script_dir, "logo.png")
    
    # Output paths
    output_dir = os.path.join(
        project_root, "Convertify", "Assets.xcassets", "AppIcon.appiconset"
    )
    icns_path = os.path.join(
        project_root, "Convertify.app", "Contents", "Resources", "AppIcon.icns"
    )
